...
## Нормативные правила
- RULE: IF рядом с изменяемым кодом есть README или другой docs-файл THEN агент MUST прочитать его до начала правок.
- RULE: После правок кода в соответствующем модуле агент MUST обновить затронутый локальный README/docs в том же изменении.
- RULE: Обновление локального README/docs MUST включать версию, дату изменения и лаконичное описание изменений.
- RULE: Локальный README/docs MUST отражать фактическое текущее поведение кода; устаревшие инструкции и противоречия MUST NOT оставаться.
- RULE: Для модульных директорий (например, components, utils, hooks, services) проект SHOULD поддерживать индивидуальные README как точку входа по назначению и использованию.
