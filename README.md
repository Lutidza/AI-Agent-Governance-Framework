...
- предоставляет диалоговый справочник команд (`aafg help`, `aafg command <ID>`, `aafg defaults`, `aafg <path> mode=review|fix`, `aafg:test <path> [fix|review]`);
...
## Диалоговый Справочник Команд AAFG

Каталог команд хранится в `aagf/docs/spec/project/commands.catalog.yaml` и валидируется по schema `aagf/docs/spec/schemas/command-catalog.schema.json`.

Быстрый просмотр каталога:

- `npm --prefix aagf run docs:commands`

Отладочный dry-run разбор пользовательской команды:

- `npm --prefix aagf run docs:command-debug -- --input "aafg:test aagf/tools/src/tests/detect-contract.ts fix"`

Базовые команды в диалоге специалиста:

- `aafg help`
- `aafg command <ID>`
- `aafg defaults`
- `aafg <path> mode=review [rule=<RULE_ID>] [profile=<PROFILE_ID>]`
- `aafg <path> mode=fix [rule=<RULE_ID>] [profile=<PROFILE_ID>]`
- `aafg:test <path> [fix|review]`

Правило безопасности:

- режим `mode=fix` MUST выполняться только после явного подтверждения специалиста; `aafg:test` всегда работает как dry-run без изменений файлов.
