...
- Runtime adapter-выход MUST включать индекс `rules/README.md` и отдельные rule-entry файлы `rules/<section>/<rule-id>.md`; монолитный единый rules-файл MUST NOT быть целевым форматом.
...
- В целевом проекте доступны `git`, `node`, `npm`.
- Детекция стека доступна через MCP tool `detect_stack_deep`.
- Рабочая директория агента — корень целевого проекта.
- В PowerShell предпочтительно использовать `npm.cmd`.

### Операционный режим (обязательно)

- Агент MUST работать по фазам `Install -> Detect -> Confirm -> Compose -> Generate -> Sync -> Lock`.
- Агент MUST запускать `Detect` через MCP (`detect_stack_deep`) и MUST публиковать `detect-started` + structured summary (`stack.*`, `unknown-levels`, `low-confidence`).
- Агент MUST проходить explicit gate `confirm` или `edit` перед `Lock`.
...
### Пошаговая установка и детекция (PowerShell)

1. Подключить дистрибутив `aagf/` в корень целевого проекта:
...
2. Установить зависимости и проверить source-слой:
...
4. Пройти Confirm gate:

- Вариант `confirm` (рекомендуется):

```powershell
npm.cmd run docs:bootstrap -- --guided --detect-action confirm
```

- Вариант `edit` (пример ручной коррекции стека):
...
5. Применить изменения в spec-слой и зафиксировать lock:
...
6. Проверить генерацию и отсутствие drift:
...
7. Выполнить sync runtime-контуров (сначала dry-run, затем apply):
...
- `.aiassistant/rules/README.md` + модульные файлы `.aiassistant/rules/<section>/<rule-id>.md`
- `.cursor/rules/README.md` + модульные файлы `.cursor/rules/<section>/<rule-id>.md`

### Подсказка Для Уже Установленного AAGF

Если ваш агент уже установил AAGF самостоятельно, отправьте ему следующий промпт:
