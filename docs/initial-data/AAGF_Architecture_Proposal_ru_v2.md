...
4. До старта реализации target-aware генерации MUST быть синхронизированы `docs/initial-data/AAGF_Architecture_Proposal_ru_v2.md`, корневой `README.md` и `aagf/docs/README.md`.
...
8. Диалоговые команды MUST описываться machine-readable каталогом `aagf/docs/spec/project/commands.catalog.yaml` с schema-валидацией и явными safety-gates для режимов `review/fix`.
...
4. **Compose**: выбрать подключаемые наборы правил, ролей, prompts и project-overrides, а также зафиксировать командный контракт (`commands.catalog.yaml`, defaults `mode=review`, confirm-gate для `fix`).
...
- `aagf/docs/spec/project/commands.catalog.yaml` — machine-readable каталог диалоговых команд (`help/command/defaults/file review|fix/docs sync/test`).
- `aagf/docs/spec/schemas/command-catalog.schema.json` — schema-контракт каталога команд и safety-gate полей.
...
13. Зафиксировать command catalog + schema и поддерживать debug-алиас `aafg:test <path> [fix|review]` для dry-run проверки маршрутизации без правок файлов.
...
| `docs/initial-data/**` | Референсный слой | Reference inputs | Входные материалы анализа и проектирования, не часть продуктового дистрибутива |
...
| `aagf/docs/spec/project/commands.catalog.yaml` | Product machine-readable command contract | Product source layer | Каталог диалоговых команд AAFG с режимами `review/fix`, параметрами и safety-gates |
| `aagf/docs/spec/schemas/command-catalog.schema.json` | Product schema contract | Product source layer | Валидация структуры каталога команд и обязательных полей |
...
- `aagf/docs/spec/project/commands.catalog.yaml` — machine-readable справочник диалоговых команд AAFG;
- `aagf/docs/spec/schemas/command-catalog.schema.json` — schema-контракт для каталога команд.
...
Этот скелет формирует `separation of concerns`: `docs/initial-data` как reference-контур, `root/meta` как control plane развития framework, `aagf/` как delivery plane (продуктовый дистрибутив), `aagf/docs/spec` как source layer дистрибутива, `aagf/docs/human` и `aagf/docs/adapters` как производные слои, `/.aiassistant` и `/.cursor` как активные runtime adapter layers проектного инстанса.
...
4. Сверить диалоговый контракт команд в `aagf/docs/spec/project/commands.catalog.yaml` и проверить отладочный dry-run (`aafg:test <path> [fix|review]`).
5. Синхронизировать `aagf/docs/adapters/jetbrains/**` в `/.aiassistant/**` и `aagf/docs/adapters/cursor/**` в `/.cursor/**` только по явному подтверждению.
6. Подключить Prompt Library и минимальные workflow prompts через активные runtime-слои выбранной IDE.
7. Настроить operational policy команды: режимы, approvals, reasoning, handoff.
8. Добавить/обновить профильные документы в `aagf/docs/spec/**`, затем синхронизировать `aagf/docs/human/**`.
9. Пройти integration checklist: видимость правил, отсутствие конфликтов, корректная синхронизация всех слоев (`spec`/`human`/adapter), работоспособность prompts.
...
|---|---|---|---|
| `v1 / Foundation` | Сразу заложить declarative-first основу + продуктовый дистрибутив `aagf/` + dual-target adapters (JetBrains, Cursor) | `README`, `AGENTS`, `docs/initial-data/**`, `aagf/package.json`, `aagf/tools/**`, `aagf/docs/README.md`, `aagf/docs/AGENTS.md`, `aagf/docs/spec/**`, `aagf/docs/human/**`, `aagf/docs/adapters/**` | Дистрибутив `aagf/` можно внедрить в проект и получить предсказуемое базовое поведение на едином machine-readable source |
...
- держать диалоговые команды в `aagf/docs/spec/project/commands.catalog.yaml` и проверять маршрутизацию через dry-run алиас `aafg:test`, не изменяя файлы на этапе отладки;
