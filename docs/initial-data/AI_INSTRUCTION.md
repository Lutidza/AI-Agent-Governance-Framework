...
* Бэкенд работает на PostgreSQL, GraphQL в проекте не используется (только REST/Local API).
...
* MUST NOT хардкодить, игнорировать типизацию и использовать другие костыли.
* Фиксируй в памяти и помечай в комментариях стабильные версии кода, которые согласованы.
* MUST NOT вставлять заглушки в ответах с кодом, например:
...
* Код правь напрямую в файлах; в ответах не вставляй содержимое файлов, если только явно не нужно показать короткий фрагмент.
* Индексные либо файлы типов называем в формате `feature.index.ts` / `feature.types.ts` (пример: `srp.index.ts`, `srpRoutes.types.ts`), чтобы контекст был понятен из названия.
* Файлы хелперов и утилит называем с суффиксами `featureHelper.ts` / `featureUtil.ts` (например, `routeOrderingHelper.ts`), чтобы назначение было очевидно из имени.
* Если рядом с редактируемым кодом есть README или другой docs-файл, прочитай его перед началом работы и обнови после правок (версия, дата изменения, описание изменений, лаконичное описание изменений).
* Индексные файлы называем в формате `feature.index.ts` (например, `srp.index.ts`), чтобы из названия было понятно, чей это индекс.
